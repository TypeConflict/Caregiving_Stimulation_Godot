...
# 👉請填入你的 API 金鑰
client = openai.OpenAI(api_key="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")
...
def save_experiment(dialogue_history, system_info, target_info, elder_role_info, judge_results, emotion_history):
    """
    將實驗結果保存到JSON檔案
    """
    ...
    filename = f'experiments/experiment_{timestamp}.json'
...
with open('scenario.json', 'r', encoding='utf-8') as f:
    ...
    initial_dialogue = scenario['initial_dialogue']  # 從JSON讀取初始對話
...
"""
        ...
        # 📌 Step 2: 再由阿嬤生成下一句
        # 只取最近三輪的對話
        recent_dialogue = dialogue[-6:] if len(dialogue) > 6 else dialogue[1:]  # 每輪有兩條訊息，所以取最後6條
        conversation_history = []
        for msg in recent_dialogue:
            if msg['role'] == 'user':
                conversation_history.append(f"你剛才說：{msg['content']}")
            else:
                conversation_history.append(f"你的照護者說：{msg['content']}")
...
我們雙方都會以第一人稱進行。請你以阿嬤的身份，參考我說的最後一句話，用第一人稱小說體說一句對我自然的回應：
"""
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "你是一位台灣阿嬤，正在和照護者對話。用小說體說話，每次只說一句。"},
                {"role": "user", "content": elder_response_prompt}
            ],
            temperature=0.8
        )
        ...
        elder_response = response.choices[0].message.content.strip()
        ...
        # 判斷是否需要更新情緒比重
        if evaluate_caregiver_response_effectiveness:
            evaluation_result = await evaluate_caregiver_response_effectiveness(
                current_emotion, reply, elder_response, chr(10).join(conversation_history)
            )
            if evaluation_result == "是":
                adjust_probabilities_for_calm()  # 更新情緒機率
    ...
    # 保存實驗結果
    save_experiment(dialogue, system_info, target_info, elder_role_info, judge_results, emotion_history)
